...
export function weeklyReminderEmail(siteName: string, isoWeek: string, appUrl: string) {
  return wrapper(
    `Besoin de nettoyage — semaine ${isoWeek}`,
    `
    <p>Bonjour,</p>
    <p>Merci de renseigner dès que possible les véhicules de <strong>${siteName}</strong> à nettoyer cette semaine.</p>
...
export function newPlateNotificationEmail(params: {
  ...
  const { siteName, plate, brandModel, attentionNotes, optionNames, requesterName, appUrl } = params;
  return wrapper(
    ...
    <table style="width:100%;border-collapse:collapse;margin:16px 0;font-size:14px;">
      <tr><td style="padding:4px 0;color:#64748b;">Immatriculation</td><td style="padding:4px 0;font-weight:bold;">${plate}</td></tr>
      <tr><td style="padding:4px 0;color:#64748b;">Véhicule</td><td style="padding:4px 0;">${brandModel || "—"}</td></tr>
      <tr><td style="padding:4px 0;color:#64748b;">Options</td><td style="padding:4px 0;">${optionNames.join(", ") || "Aucune"}</td></tr>
      <tr><td style="padding:4px 0;color:#64748b;">Point d'attention</td><td style="padding:4px 0;">${attentionNotes || "—"}</td></tr>
      <tr><td style="padding:4px 0;color:#64748b;">Demandé par</td><td style="padding:4px 0;">${requesterName}</td></tr>
    </table>
    <p style="margin:24px 0;">
